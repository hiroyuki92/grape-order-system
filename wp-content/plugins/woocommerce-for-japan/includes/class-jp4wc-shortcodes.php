...
class JP4WC_Shortcodes {
	...
	public static function init() {
		...
		foreach ( $shortcodes as $shortcode => $function ) {
			add_shortcode( apply_filters( "{$shortcode}_jp4wc_shortcode_tag", $shortcode ), $function ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound -- hook name is dynamically built with jp4wc_-prefixed shortcode keys
		}
	}
    ...
    <tbody>';
		...
		foreach ( $laws_array as $key => $value ) {
			...
			} elseif ( __( 'Special conditions', 'woocommerce-for-japan' ) !== $key && __( 'Telephone', 'woocommerce-for-japan' ) !== $key && __( 'Manager Name', 'woocommerce-for-japan' ) !== $key ) {
				$no_content[] = $key;
			}
		}
		$content .= '    </tbody>
